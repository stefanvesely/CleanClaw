## User Working Rule: Approval First

Codex/AI agents must not edit files, apply patches, run builds, run tests, commit, push, install dependencies, run setup actions, or make project changes until the exact intended action has been shown to the user and explicitly approved.

Required flow:

1. Investigate only.
2. Show the proposed change in plain language.
3. If code is involved, show the exact file, method, intended code change, and why the change is needed before editing.
4. Wait for explicit approval such as "apply it", "do it", "yes patch", or "approved".
5. Make only the approved change.
6. Stop and report exactly what changed.

Every proposed change must include a clear "Why" section explaining the purpose of the change and how it connects to the user's requested outcome.

No adjacent fixes, cleanup, refactors, tests, docs, changelogs, commits, pushes, or builds unless separately approved.

If an unapproved change is made, stop immediately, show exactly what changed, and ask whether to keep or revert it.
...
1. **`cli`** â€” `test/**/*.test.{js,ts}` â€” integration tests for CLI behavior
2. **`plugin`** â€” `nemoclaw/src/**/*.test.ts` â€” unit tests co-located with source
3. **`e2e-branch-validation`** â€” `test/e2e/brev-e2e.test.ts` â€” validates a branch from source on ephemeral Brev instance (requires `BREV_API_TOKEN`)
...
- The `nemoclaw/` subdirectory has its own `package.json` and `node_modules`, while sharing the root Biome config â€” it's a separate npm project
- SPDX headers are auto-inserted by pre-commit hooks; don't worry about adding them manually
- Coverage thresholds are ratcheted in `ci/coverage-threshold-*.json` â€” new code should not decrease CLI or plugin coverage
- The `.claude/skills` symlink points to `.agents/skills` â€” both paths resolve to the same content

## Documentation

- Source of truth: `docs/` directory
- `.agents/skills/nemoclaw-user-*/*.md` is **autogenerated** â€” never edit directly
