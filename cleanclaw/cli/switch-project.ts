...
export async function switchProject(
  ...
  if (!fs.existsSync(path.join(resolved, 'cleanclaw.config.json'))) {
    logger.error(`Error: "${resolved}" is not a CleanClaw project (no cleanclaw.config.json found).`);
    logger.error('Run "cleanclaw init" inside that directory first.');
    process.exit(1);
  }
  ...
  logger.info(`Switched to ${projectName}. Last task: ${lastTask}.`);
}
